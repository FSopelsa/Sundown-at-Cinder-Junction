...
- Geometry, normal map and roughness map are original procedural work. No photo
  pixels, table fabric, background, text, watermark or third-party mesh is embedded.
- No external provider, purchased asset, paid generation or image upload was used.
- Intended use: reusable empty bag prop, with continuous opening and attachment
  anchors; not an implemented inventory system or level obstacle.
- Scale: 1 cell high, 0.62 cells wide, Y-up GLB, +X front, ground-centred origin.
  Actual physical size is unknown. Reverse surface and opening are inferred.
