...
test('zip bag opens and closes visibly on desktop and mobile', async ({ page }, testInfo) => {
  const errors = [];
  page.on('pageerror', e => errors.push(e.message));
  page.on('console', m => { if (m.type() === 'error') errors.push(m.text()); });
  await page.setViewportSize({ width: 1440, height: 1000 });
  await page.goto('/?assets&model=Prop_ZipBag');
  const viewport = page.locator('[data-asset=viewport]');
  await expect(viewport).toHaveAttribute('data-rendered-model', 'Prop_ZipBag');
  ...
  await page.screenshot({ path: testInfo.outputPath('zip-bag-closed-desktop.png') });
  await opening.focus();
  ...
  await page.screenshot({ path: testInfo.outputPath('zip-bag-open-desktop.png') });
  ...
  await page.screenshot({ path: testInfo.outputPath('zip-bag-front.png') });
  await page.setViewportSize({ width: 390, height: 844 });
  await page.locator('[data-asset=view]').selectOption('perspective');
  await opening.focus();
  await opening.press('End');
  await page.screenshot({ path: testInfo.outputPath('zip-bag-open-mobile.png') });
