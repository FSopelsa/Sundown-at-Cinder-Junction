...
import { PICKUP_DEFINITIONS, SNABBA_SKOR_SPAWN_MAX_MS, SNABBA_SKOR_SPAWN_MIN_MS } from '../../src/game/content/pickups.js';
...
test('the Snabba skor pickup is serialized and grants a temporary movement boost', () => {
...
test('Snabba skor schedules one random spawn on every level', () => {
  for (const map of LEVELS) {
    ...
    assert.equal(simulation.state.pickups[0].type, 'speed-boost');
    assert.deepEqual(
      { x: simulation.state.pickups[0].x, y: simulation.state.pickups[0].y },
      map.pickupSpawnPoint,
    );
  }
});

test('the Scrap Exchange sells Snabba skor for 200 Scrap', () => {
  const simulation = createSimulation(new GameState({ levelId: THRESHOLD_MAP.id, scrap: 250 }));
  const exchange = {
    id: 'scrap-exchange-test',
    type: 'scrapExchange',
    hp: 100,
    maxHp: 100,
    construction: null,
  ...
  simulation.state.towers.push(exchange);

  const result = simulation.dispatch('purchase-support', {
    towerId: exchange.id,
    item: 'snabbaSkor',
  });
