...
async function readKit(key) {
  ...
  if (key === MODEL_KEYS.industrialKit || key === MODEL_KEYS.zipBag || key === MODEL_KEYS.snabbaSkor) {
    ...
    assert.equal(doc.getRoot().listTextures().length, [MODEL_KEYS.zipBag, MODEL_KEYS.snabbaSkor].includes(key) ? 2 : 1, 'packed procedural maps');
...
test('all catalogue entries resolve to unique, origin-centred GLB roots with painted geometry', async () => {
  ...
  const bagReports = [
    JSON.parse(await readFile('docs/3d/zip-bag/build-report.json', 'utf8')),
    JSON.parse(await readFile('docs/3d/snabba-skor/build-report.json', 'utf8')),
  ];
  assert.equal(report.assets.length + bagReports.reduce((sum, item) => sum + item.assets.length, 0), ASSET_CATALOGUE.length);
  for (const key of new Set(ASSET_CATALOGUE.map(a => a.model))) {
    ...
    for (const entry of entries) {
      ...
      root.traverse((node) => {
        ...
        meshes += 1;
        assert.ok([MODEL_KEYS.zipBag, MODEL_KEYS.snabbaSkor].includes(key) ? node.geometry.attributes.uv : node.geometry.attributes.color, `${entry.node} material coordinates`);
      });
      assert.ok(meshes > 0, entry.node);
    }
  }
  for (const pkg of [report, ...bagReports].flatMap((item) => item.packages)) {
