...
import { setSnabbaSkorOpening } from '../../src/three/snabbaSkor.js';

async function loadBag() {
  const io = new NodeIO().registerExtensions(ALL_EXTENSIONS);
  const document = await io.read(`public${getModelAsset(MODEL_KEYS.snabbaSkor).path}`);
  const material = document.getRoot().listMaterials().find(m => m.getName() === 'SnabbaSkor_ClearPolyethylene');
  ...
  return gltf.scene.getObjectByName('Prop_SnabbaSkor');
}

test('Snabba skor has real punched geometry, a sealed base, anchors and thin-film materials', async () => {
  ...
  assert.ok(bag.getObjectByName('SnabbaSkor_ContentAnchor'));
  assert.ok(bag.getObjectByName('SnabbaSkor_HangingAnchor'));
  assert.equal(bag.userData.forward_axis, '+X');
  assert.equal(getModelAsset(MODEL_KEYS.snabbaSkor).preload, true);
...
test('bag opening deforms all six parts, preserves the base and is independent per clone', async () => {
  ...
  assert.equal(setSnabbaSkorOpening(bag, 1), 6);
  ...
  setSnabbaSkorOpening(bag, 0.5);
  bag.traverse(n => { if (n.morphTargetInfluences) assert.equal(n.morphTargetInfluences[0], 0.5); });
  setSnabbaSkorOpening(bag, -1);
  assert.equal(bag.userData.opening, 0);
  setSnabbaSkorOpening(bag, 2);
  assert.equal(bag.userData.opening, 1);
  assert.throws(() => setSnabbaSkorOpening(bag, NaN), TypeError);
  assert.equal(setSnabbaSkorOpening(new THREE.Group(), 1), 0);
});
